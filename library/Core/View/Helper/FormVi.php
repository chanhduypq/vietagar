 ...
 */
class Core_View_Helper_FormVi extends Zend_View_Helper_Abstract{
	/**
	 * @author Trần Công Tuệ
	 ...
	 * @return string $html
	 */
	public function formVi($view,$form,$numberOfElement_per_row,$echo_form_tag,$type='table'){
				...
				?>
					<table width="100%"> 
						...
						foreach ($elements as $element){	
                                                    if(strpos($element->getName(), '_cn') === FALSE && strpos($element->getName(), '_en') === FALSE){
							if($j==0){
								echo '<tr>';
							}
	...
	private function buildJqueryForElement($elements,$view){
		...
		foreach ($elements as $element){
                    //if(strpos($element->getName(), 'cn') === FALSE && strpos($element->getName(), 'en') === FALSE){
