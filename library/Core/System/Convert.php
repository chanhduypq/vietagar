...
class Core_System_Convert{
	...
	public static function Detect($input){
		...
		//echo count(self::$desc);
		for($i=0;$i<count(self::$desc);$i++){
			$j=0;
			$s = $input;
			foreach(self::$desc[$i] as $k=>$v){
				$s = str_replace($v,"",$s);
				if(strlen($s)<$mark){
					$detectcode = $i;
					$mark = strlen($s);
	...
	public static function ConvertToUnicode($input){
		$code = self::Detect(substr($input,0,255));
		...
		$i=0;
		foreach(self::$desc[$code] as $v){
			$input = str_replace($v,"<~>".$i."<~>",$input);
			$i++;
		}
		$i=0;
		foreach(self::$desc[$code] as $k=>$v){
	...
	public static function recreate(){
		for($code=0;$code<count(self::$desc);$code++){
			...
			for($i=0;$i<count($source);$i++){
				for($j=$i;$j<count($source);$j++){
					...
					if(strlen(self::$desc[$code][$i])==strlen(self::$desc[$code][$j])){
						if(strcmp($source[$i],$source[$j])<0){
							$tmp = $source[$i];
							$source[$i] = $source[$j];
							$source[$j] = $tmp;
							$tmp = self::$desc[$code][$i];
							self::$desc[$code][$i] = self::$desc[$code][$j];
							self::$desc[$code][$j] = $tmp;
						}
					}else{
						if(strlen(self::$desc[$code][$i])<strlen(self::$desc[$code][$j])){
							$tmp = $source[$i];
			...
			$rebuildarr = array();
			for($i=0;$i<count($source);$i++){
				$rebuildarr[$source[$i]]=self::$desc[$code][$i];
			}
